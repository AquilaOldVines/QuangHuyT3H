...
import com.example.chitieucanhan.viewmodel.userviewmodel.UserRepository
...
class ChiFragment(private val checkMouth : Int) : BaseFragment<FragmentShowChitietBinding>() {
    ...
    private val userViewModelChi : UserViewModel by lazy {
    ...
    override fun initView() {
        ...
        userViewModelChi.getAllUser().observe(this, Observer { value ->
            ...
            var dataSet = PieDataSet(entries, "Tiền chi ${checkMouth}")
            ...
            value.forEach { item ->
                ...
                if (item.dateuser.month == checkMouth-1 && item.moneyChiUser != 0f){

                    var dataPie : DataOfPiechart = DataOfPiechart(item.typeUser,item.nameTypeName, item.moneyChiUser, 0)
